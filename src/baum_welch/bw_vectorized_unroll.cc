...
inline void forward_backward(double* forward, double* backward, int M, int N, int T,
    ...
    __m256d acc_v2, B_vect2, temp1, temp2;
    ...
    for (int t = 1; t < T; t++) {
        ...
        for (int i = 0; i < M; i+=8) {
            ...
            acc_v = _mm256_set1_pd(0.0);
            acc_v2 = _mm256_set1_pd(0.0);

            B_vect = _mm256_load_pd(B + observation_seq[t]*M + i);
            B_vect2 = _mm256_load_pd(B + observation_seq[t]*M + i + 4);

            for (int j=0; j<M; j++) {

                A_vect = _mm256_load_pd(A + j*M + i);
                A_vect2 = _mm256_load_pd(A + j*M + i+4);

                temp = _mm256_set1_pd(forward[(t-1)*M + j]);

                acc_v = _mm256_fmadd_pd(temp, A_vect, acc_v);
                acc_v2 = _mm256_fmadd_pd(temp, A_vect2, acc_v2);

            }

            temp = _mm256_mul_pd(B_vect, acc_v);
            temp2 = _mm256_mul_pd(B_vect2, acc_v2);

            _mm256_store_pd(forward + t*M + i, temp);
            _mm256_store_pd(forward + t*M + i+4, temp2);
    ...
    for (int t = T-2; t >= 0; t--) {
        for (int i = 0; i < M; i+=4) {
            ...
            temp1 = _mm256_mul_pd(sc_vector, sum_vector);
            _mm256_store_pd(backward + t*M + i, temp1);
...
void BaumWelchCVectUnroll::operator()() {
