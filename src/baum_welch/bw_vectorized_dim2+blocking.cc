...
static void update_and_check(double* forward, double* backward, int M, int N, int T,
  ...
  double sum[16*16];  // CHANGE PARAMETER
  int blocking_size = 16; // CHANGE PARAMETER
  ...
  for (int i = 0; i < M; i += blocking_size) {
    for (int j = 0; j < M; j += blocking_size) {
      ...
      for (; t < T - blocking_size - 1; t += blocking_size) {
        ...
        for (int i_ = i; i_ < i + blocking_size; i_ += 4) {
          ...
          for (int j_ = j; j_ < j + blocking_size; j_ += 4) {
          ...
          for (int t_ = t; t_ < t + blocking_size; t_++) {
            __m256d fwd_vec1 = _mm256_set1_pd(forward[t_*M + i_]);
            __m256d fwd_vec2 = _mm256_set1_pd(forward[t_*M + i_+1]);
            __m256d fwd_vec3 = _mm256_set1_pd(forward[t_*M + i_+2]);
            __m256d fwd_vec4 = _mm256_set1_pd(forward[t_*M + i_+3]);

            __m256d bwd_vec1 = _mm256_load_pd(backward + (t_+1)*M + j_);
            __m256d b_vec1 = _mm256_load_pd(B + observation_seq[t_+1]*M + j_);
          ...
          _mm256_store_pd(sum + k*blocking_size + l, _mm256_add_pd(acc1, _mm256_load_pd(sum + k*blocking_size + l)));
          _mm256_store_pd(sum + (k+1)*blocking_size + l, _mm256_add_pd(acc2, _mm256_load_pd(sum + (k+1)*blocking_size + l)));
          _mm256_store_pd(sum + (k+2)*blocking_size + l, _mm256_add_pd(acc3, _mm256_load_pd(sum + (k+2)*blocking_size + l)));
          _mm256_store_pd(sum + (k+3)*blocking_size + l, _mm256_add_pd(acc4, _mm256_load_pd(sum + (k+3)*blocking_size + l)));
        }
    ...
    for (int i_ = i; i_ < i + blocking_size; i_ += 4) {
      ...
      for (int j_ = j; j_ < j + blocking_size; j_ += 4) {
        ...
        _mm256_store_pd(A + i_*M + j_,     _mm256_mul_pd(_mm256_load_pd(A + i_*M + j_),_mm256_add_pd(acc1, _mm256_load_pd(sum + k*blocking_size + l))));
        _mm256_store_pd(A + (i_+1)*M + j_, _mm256_mul_pd(_mm256_load_pd(A + (i_+1)*M + j_),_mm256_add_pd(acc2, _mm256_load_pd(sum + (k+1)*blocking_size + l))));
        _mm256_store_pd(A + (i_+2)*M + j_, _mm256_mul_pd(_mm256_load_pd(A + (i_+2)*M + j_),_mm256_add_pd(acc3, _mm256_load_pd(sum + (k+2)*blocking_size + l))));
        _mm256_store_pd(A + (i_+3)*M + j_, _mm256_mul_pd(_mm256_load_pd(A + (i_+3)*M + j_),_mm256_add_pd(acc4, _mm256_load_pd(sum + (k+3)*blocking_size + l))));
      }
    }
  }
}
  ...
  for (int i = 0; i < M; i += 4) {
    ...
    for (int j = 0; j < M-3; j += 4) {
      _mm256_store_pd(A + i*M + j, _mm256_mul_pd(f1, _mm256_load_pd(A + i*M + j)));
			_mm256_store_pd(A + (i+1)*M + j, _mm256_mul_pd(f2, _mm256_load_pd(A + (i+1)*M + j)));
			_mm256_store_pd(A + (i+2)*M + j, _mm256_mul_pd(f3, _mm256_load_pd(A + (i+2)*M + j)));
			_mm256_store_pd(A + (i+3)*M + j, _mm256_mul_pd(f4, _mm256_load_pd(A + (i+3)*M + j)));
    }
  }
...
void BaumWelchCVectBlocking::operator()() {
